...
- **`cold-audit-modules.mjs` — the coverage gate did not run whenever the check found anything.** The 80% gate added in v0.1.50 was written as an `else if` after the suspicious-call-site branch, so a single candidate skipped it entirely. A run that examined 1 of 5 modules printed its own `1/5 examined` immediately above `PASS` and exited 0. This is the third appearance of the failure mode v0.1.50 was written to close, and a gate hanging off an `else` branch is not a gate. Now evaluated unconditionally.
