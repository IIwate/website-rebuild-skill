...
**它盯着的是"对不对"，不是"像不像"**。最近一个自研 WebGL 引擎站的复刻，三条路由做到了**跨侧逐像素一致**（`meanAbsDiff 0.00`）。
...
| | 扒站 / 镜像 | 工程化复刻 |
|---|---|---|
| **你拿到的 JS** | 压缩混淆后的产物，一行几十万字符 | **可读源码**，每一行标注它来自源站的哪一行 |
...
| 复刻出来 | 从中分离出 **14,232 行**站点自研代码（可读、每行标注源站行号），three.js 按取证到的版本从 npm 装回；464 个模块逐个清点无遗漏 |
...
| Lusion | [lusion.co](https://lusion.co/) | 创意工作室官网，1.25 MB 自研 WebGL 引擎 + 156 个 shader，三条路由**逐像素一致** |
