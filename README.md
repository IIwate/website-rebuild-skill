...
**四个阶段**：**镜像取证**（第一时间把整站抓成只读证据，断网能跑通才算过）→ **读懂源码**（把压缩代码展开成可读形态，此后每一行都能指回出处；先写逆向笔记，再写任何代码）→ **逐行移植**（每处改动都先找到源码出处；连 bug 和怪写法也照抄不修）→ **验证收口**（按站点特性选择验收门，最后逐模块清点）。

**六条纪律**（违反任何一条都会在后面以 bug 的形式偿还）：

1. 镜像只读，永不修改
2. 以源码为唯一裁决，不凭肉眼调效果
3. 源站有的都要有，源站没有的不发明
4. bug、死代码、怪写法照抄不修
5. 有意的差异必须登记——**没登记的差异一律算 bug**
6. 代码与文档同一次提交
