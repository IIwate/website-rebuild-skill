...
- **可行的规避**：`probe.mjs` / `pixelcompare.mjs` 新增 `--format png|jpeg|webp --quality N`（默认仍 PNG 以保字节保真；`probe.mjs` 的 `--shot x.jpg` 会按扩展名自动切 jpeg）。**什么时候必须降**：视口 ≳ 1500×900 且内容是照片/噪声类时 PNG 可能到不了岸，可改 `--format jpeg --quality 92`（该机实测 58ms/张）。⛔ **字节门保持 PNG，有损格式（jpeg/webp）只进像素/指标门**——静止态连拍逐字节相同只能证明该状态下编码器输出稳定，不能把有损格式升级为字节保真证据。
...
| `scripts/verify-ledger.mjs` | 可选坐标台账门: 逐路径核对行号与 Needle | 坐标系建立或变更后 | 通用工具 | 需调用方配置 |
| `scripts/verify-sourceified-tokens.mjs` | 可选源码化 token 门: 只接受登记的标识符与 shorthand 差异 | M(n+1) | 通用工具 | 需调用方配置 |
