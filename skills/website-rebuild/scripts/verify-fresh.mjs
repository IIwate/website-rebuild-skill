#!/usr/bin/env node
/**
 * Rebuild a selected entry into a temporary file and compare output bytes.
 * Checks the generated bundle against dist and the copy served from site. Uses
 * the project's esbuild binary or an explicit --esbuild path, with the IIFE bundle
 * options below; other build configurations need a corresponding project check.
 *
 * Modification times alone do not establish freshness. In one recorded case,
 * dist/site.js was a day older than src, but rebuilding produced identical bytes
 * because the source change removed an unreferenced file.
 *
 * A missing entry reports SKIPPED with exit 0. That result does not validate a
 * bundle; check the configured path or use the checks appropriate to the project.
 *
