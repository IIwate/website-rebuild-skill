...
> **何时加载本文件**:第 0 步判级为 A/B 后**立即**加载并动工. 镜像先于一切分析--历年获奖站 29% 已消失(域名易主/平台回收/抢注/路径移除/HTTP 200 的原地替换五种形态俱全),"第一时间全站镜像作只读证据"不是最佳实践, 是存档恢复行为[probe].M0.5 断网跑通是阶段前置检查: 镜像不可跑, 不得进入逆向与移植.

### 0.10  弱挑战标记与文档类型判断[v0-optimus]

真实性检查用两档标记查挑战页: 强标记(只可能出现在挑战体里)对所有文本文件生效; 弱标记(在真页面上也会出现)只对**小于 32 KB 的文档**生效.
 ...
 这里的误报特别贵, 检查自己的注释已经写明了原因:**它训练你跳读这道检查的输出**,而那正是当初 43 份挑战页能活下来的方式. 修完必须回过头验证弱标记仍会在真挑战页上触发.

### 0.11  防目录穿越的守卫写成 `includes("..")` 会误报合法文件名[eightdesign]

镜像服务器对一个**磁盘上确实存在**的字体返回 404.根因在守卫:

```js
if (clean.includes("..")) return null;   // Substring matching also rejects valid asset names.
```
...
**1) CDP 补录只跑了 `/`.** 其余路由有各自在运行时拼出来的资源, 静态提取看不见. **镜像侧报错说明参照本身不完整**,此时任何跨侧数字都不该读(实证:`case-studies/mirroring.md` §0.12).

 **要对比哪条路由, 就先给哪条路由跑第二遍.** 不是"跑一次首页就代表全站".

**2) 那 4 条 404 是 Next 的图片优化端点**:
...
处置: 把该端点的响应按 `url+w+q` 三元组入库(`@@` 查询编码天然支持),并在偏差表里写明**镜像覆盖的是哪几组 `w`**--因为覆盖面就是"你能对比哪些视口"的上限.

### 0.13  核对捕获内容与目标资源[eightdesign]

 一个 diff 的两边各自是什么, 是 diff 结论的前提.**取证脚本必须先断言"我拿到的东西
像那份资源"**--尺寸量级, 状态码, 内容类型, 任一条不合就停下, 而不是把它喂进比对.
这与 `verify-mirror` 的真实性检查是同一条约束, 只不过那道检查管的是**存进来的**字节,
...
https://host/static/media/9010da...       // The previous push contained /_next.
https://host/9dc1a6fb114b646f-s.p...      // The previous push contained the entire path prefix.
```
...
"nobody can name a URL for".(实证:`case-studies/mirroring.md` §0.16)
 ...
 写盘与清单更新之间发生错误时, 工具应报告未登记文件, 并保留已完成请求的记录. 验证需要检查磁盘与清单的一致性.
...
而真正那条 URL **只有抓包看得见**(实证:`case-studies/mirroring.md` §0.18).
 ...
 值得记住这条资产的处境: 闭包检查不会报缺(完整 URL 在字节里根本不存在),
静态外联检查也不会报(同理),**只有资源级探针看得见它**.§1.6 的四类断言互补,
这是第四类唯一能抓到的那一格.
...
从裸请求里回答其中任意一个, 正是单射检查存在要抓的那种坍缩. 唯一就给, 多个就 404 让检查说话.
...
3.  **无扩展名的服务端路由要按 manifest 记录的 content-type 提供服务**:`/api/_auth/session`
...
真实性检查的 ttf 魔数现在认 `00010000`, `true`, `OTTO` 三种.

>  **目标站已死?** 本文假定有活源可爬. X 类(原站消失)的存档恢复走
> [archival-rescue.md](archival-rescue.md):`wayback-mirror.mjs` 从 Internet Archive
> 产出与本文同构的标准镜像, 唯一的语义差异是"洞是既成事实, 登记即交付".

## 0. 三条基础原则

1. **保存原始采集内容**: `mirror/` 作为采集快照保留. URL 本地化、探针和平台适配放在服务层或派生产物中, 使后续比较可以追溯到原件[samsy][noomo][lando].
...
**目录职责**: `mirror/` 保存采集快照, `port/` 保存移植基线, `src/` 保存可维护源码, `dist/` 保存部署产物. 工作区可用符号链接或服务映射复用大资产; 自包含交付须包含已声明范围内的运行依赖, 不依赖开发工作区路径. 见 `asset-management.md` 与 `readable-source.md` §2[oryzo].

## 0.9  三个只有大型商业站才会暴露的镜像缺陷[airpodspro]

下面三处缺陷此前从未触发, 它们的共同点是:**都需要某种"前面几个站恰好没有"的形态**才会现身.(实证:`case-studies/mirroring.md` §0.9)

**1) `--scope` 有个洞:`.html` 被当资产, 绕开页面范围.**
 ...
 **副作用是好的**:从"静默过量抓取"变成"闭包检查明确地报告有引用离开了范围".

**2) 爬虫与服务器对同一个 URL 算出两个文件名.**
带查询串的**目录式** URL(`/path/?a=1&b=2`)上,"挂 `@@query` 后缀"与"补 `/index.html`"的**先后顺序**两边是反的: 爬虫写 `…/name@@query/index.html`,服务器找 `…/name/@@query`.无尾斜杠时两者一致,**只有目录式 URL 才分叉**.症状是断网检查 404 而文件就在盘上.
 **v0.1.11 早有"同一个答案在工具链里只能有一份实现"这条约束, 而这两份实现就在同一个文件里, 隔了 50 行. 同文件不等于同一份实现.**
...
采集方法取决于资源的表达方式. 静态页面可直接扫描 HTML/CSS; kimi 的部分 Next/RSC 资源位于 hash chunk 和 Flight 转义字符串中, 仅跟随 HTML 链接未能发现. 按实际缺口组合下列发现方式与响应检查[kimi].
...
rogier 首创, noomo/lando 三代项目实践传承的骨架[rogier][noomo][lando]:
...
静态解析对**运行时拼接的 URL**天然漏检. headless Chrome 实跑全路由 × 桌面/移动双视口, 走完整个滚动/交互流程, 用 CDP 记录实际发出的同源请求, 与磁盘 diff 出 GAP 清单逐项补录[kimi]:
...
```bash
# Referenced assets. Adjust hash length to the target builder; Vite often uses eight characters.
...
**差集为空是 M0 验收条件之一(§10)**;差集里若确有故意不入库的外部 chunk,按 §6 外部依赖决策表逐条登记, 不许无声留着.
...
3. 独立验证脚本用裸 fetch 断言每条重定向的**状态码本身**--Next 的 `permanent: true` 发 308 而源站发 301,检查必须断言状态码而不只断言"有重定向"[kimi].

清单应区分目标文档、重定向和挑战页. 不将重定向目标的正文登记为来源 URL 的成功响应. 核实错误记录后更正文件映射, 保留必要的失败采集证据, 将重定向记入 `redirects.tsv`, 见 `case-studies/mirroring.md` §2[objectarchive].
...
| `netcapture.tsv` | 抓包 HAVE/GAP 对账表 | GAP=0 是 M0 验收条件之一[kimi] |
...
例外条款:**后代演进为"干脆不改磁盘"**.如确实不得已改磁盘, 必须照 rogier 的登记约束执行.(实证:`case-studies/mirroring.md` §4)

## 5. 断网跑通验收检查(M0.5, 阶段前置检查)

"镜像可跑才能当对比基准, 且实跑必然暴露静态解析盲区"--隐藏关键步[lando].**先过 §5.1 的镜像自检检查**(本检查的每一项都拿镜像当输入, 镜像错了它照样能全部通过),再用 `scripts/serve.mjs` 提供服务镜像, 断网(或禁外联监控下)执行:

验收标准(全部满足才验收):
- **零 404**.
- **零控制台错误**:全路由 + 404 页跑 `scripts/probe.mjs` 探针全 CLEAN,首页含**全滚动**[lando].
- **零外联**:无任何对源站/CDN 的真实网络请求[samsy].
- **重定向断言**:裸 fetch 独立跑, 用 `scripts/verify-routes.mjs` 对镜像提供服务执行路由/重定向/状态码契约[kimi].
- **关键流程走通**:首访交互流程实际走一遍[samsy].
- **GAP=0 对账**:netcapture 对账表无未核对条目[kimi].

实跑必然暴露盲区并当场补录, 这是预期内流程而非失败(实证:`case-studies/mirroring.md` §5).

M0.5 之后,`serve.mjs` 终身兼任后续所有对比的"源站参照服"(如 `PORT=3200 SERVE_ROOT=mirror`)[noomo].

### 5.1 镜像要有属于自己的检查: 下游全部通过证明不了镜像对[objectarchive]

**下游所有检查测的是"渲染得出来吗",不是"字节对不对".** 零 404, 零控制台错误, 零外联, 像素对比--每一道都跑在镜像**之上**, 拿镜像当输入. 于是镜像是全项目的证据基准, 却是**唯一没有独立验收**的一环: 镜像错了, 下游照样可以全部通过.

 **覆盖案例**: objectarchive 未采集约 60% 的目标文件时, 五项检查持续通过四个里程碑, 见 `case-studies/mirroring.md` §5.1. 完整性应相对于已授权、已声明的采集范围定义. 未采集项逐条记录 URL、原因及影响, 不能计为已获取; 私有保存不自动获得复制许可, 见 `legal-and-deploy.md` §0.2.

 **实证二: 镜像里有 43 份 bot 挑战页, 而镜像检查是 PASS 0**[objectarchive](详见 `case-studies/mirroring.md` §5.1).

**账本记的是"你抓到了什么",从不记"它是不是你要的那个".**

变换命中数在该案例中暴露了输入文档变化. 其他类型的错误响应仍需要对应检查.

由此的命题, 也是下面"真实性"那一项断言存在的理由:**一个 HTTP 200 不是"你拿到了那个资源"的证据.** 反爬挑战页, 同意墙, 地区拦截页, catch-all 补充检查页**全部以 200 + `text/html` 返回**,而账本 / 单射性 / 闭包 / 覆盖度**每一项都在诚实地校验一份错误的内容**.

**实证三**:图片 CDN 是**查询参数化的变换接口**--`x.jpg?width=320` / `?width=600` / `?width=1200` 是三份不同字节的资源. 而 url→路径映射只看 `pathname`,三个尺寸**坍缩成同一个文件**;serve 端每个 `?width=` 又都回那同一个文件, 页面照样把图渲染出来 → **零 404 检查在错镜像上通过**.这类错不会在 M0.5 暴露, 会一路活到像素对比才以"某张图糊了 / 尺寸不对"的形态出现, 那时归因成本已经翻几倍(实证:`case-studies/mirroring.md` §5.1).

因此镜像自检检查与 M0.5 断网检查**并列, 且跑在它之前**.断言面五项, 而其中**"真实性"那一项与其余四项正交**:其余四项校验的是"**账本与磁盘是否自洽**",它校验的是"**磁盘上的东西是不是你以为的那个东西**"--其余四项全部通过说不出它的任何事(实证二):
...
- [ ] **闭包完整性**:引用集 − 磁盘集 = ∅(§1 第四遍),差集里每一条在 `external.txt` 有决策.** 审一道检查先问它的输入怎么被界定, 再问它的判据对不对**:这一项已经两次漏报在输入上而不是判据上(实证:`case-studies/mirroring.md` §5.1).修法都在 `scripts/lib/extract-refs.mjs`:**爬虫与检查共用同一份判定**,"什么算文本"按 **声明的 content-type → 扩展名 → 内容嗅探** 三级决定, 不是一张扩展名表[objectarchive].
- [ ]  **真实性(AUTHENTICITY):磁盘上的东西是不是你要的那个**.至少两条硬断言 + 一条线索:
  - **挑战 / 拦截正文匹配**(失败):Cloudflare(`_cf_chl_opt` / "Just a moment" / "Checking your browser"), Imperva/Incapsula, Akamai, Sucuri, PerimeterX 等已知挑战页正文.**判据要分强弱**--厂商专有标记任意体量都判为失败, 而"页面里有 reCAPTCHA / 有 WAF 脚本"这类**真页面也会命中**的弱标记, 只在**整份文档很小**时才算数(挑战页**就是**整份文档, 真页面只是包含一个控件).**表必须可扩展**:每家厂商都在造新的, 脚本留 `--interstitial-extra` 口子, 见到一次就登记一条.
  - **声明类型与魔数字节对照**(失败):声明是图片/字体/媒体/脚本的, 正文必须匹配对应魔数; 声明是二进制而正文是 HTML 文档的一律红. 这一条抓的是"拒绝页 / 登录墙 / SPA 补充检查页顶着资产 URL 落盘". **判据的依据是源站声明的 content-type,不是 URL 的扩展名**:扩展名是源站自己的命名选择, 不承诺任何事. 改成对着账本的 type 比之后, 误报消失而判据**更严**.
  - **同类体量离群**(**只报线索, 不判为失败**):挑战页 9.5 KB vs 真文档 300 KB+ 差两个数量级, 这是抓"还没有人有正则的那一类挑战页"的补充检查.**不判为失败是有意的**:查询参数化的 CDN 上"同类"永远不精确(一张纯色卡与一张摄影共享 `?width=1200`,差 200 倍是诚实的),判为失败只会换来一个调参旋钮和一张豁免表--正是 `gate-failure-modes.md` §1 说检查是怎么坏掉的那两条路. 同类分组必须带上**变换参数本身**(`?width=` 之类)(实证:`case-studies/mirroring.md` §5.1).
...
- **小响应告警**:bundle 响应 <1KB 极可能是拒绝页--按字节数守卫, 触发即补齐 Referer 重试[probe].**绝对阈值只对最极端的一档有效**:9.5 KB 的挑战页顶替 300 KB 的真文档时它一声不响, 所以镜像检查里的形态是**同类体量离群**而不是固定字节数(§5.1"真实性")[objectarchive].
...
按目标站机制核对下列资源类别; 它们在既有静态采集中出现过遗漏, 见 `case-studies/mirroring.md` §8[oryzo][samsy]:

- [ ] worker 运行时才 fetch 的文件(WASM 排序 worker, baker.worker)[oryzo][samsy]
- [ ] 懒加载资源(画廊图片, preloader 图, 懒加载 chunk)[oryzo][samsy]
- [ ] **流媒体播放列表与分片**:现有静态引用扫描器不展开 HLS 播放列表, 需要用 `scripts/gapfill-video.mjs` 递归获取 `.m3u8` 引用的备用轨道、初始化段、密钥和媒体分片. DASH 使用 MPD 清单, 需相应的解析工具[racingshop](实证:`case-studies/mirroring.md` §8).
...
- [ ] 前三遍共同盲区: 既不字面出现, 又不被抓包触发, 也非模板拼接的 chunk → 用第四遍静态闭包校验补充检查[shopifydesign]
...
- [ ] **HTML/CSS/JS 之外的文本格式**:`.atom` / `.rss` / `.xml` / sitemap / `.txt` / `.webmanifest` / `.map`,以及**无扩展名的路由**与源站 MIME 表不认识的扩展名(服务端一律回 `application/octet-stream`).这些文件**装满商品链接与 CDN 图 URL**,但"什么算文本"如果是一张扩展名白名单, 它们**从来不会被任何一侧打开**--而闭包检查看不出来, 因为爬虫与检查共用同一张白名单. 做法: 判定按 **声明的 content-type → 扩展名 → 内容嗅探** 三级走,`octet-stream` 当**没有声明**处理(它是"服务器不知道",不是"这是二进制"),且**爬虫与闭包检查共用同一份实现**[objectarchive]
- [ ] **查询参数化的资产变换接口**:图片 CDN 把尺寸/裁剪/格式写在 query 里(`x.jpg?width=320|600|1200`, `?crop=center`, `&format=webp`),**同 pathname 不同字节**.按 pathname 落盘会让整组变体坍缩成一个文件, 而下游零 404 检查仍可能通过(§5.1).做法: 映射与落盘**查询感知**,并把"同 pathname 多变体"单独清点[objectarchive]
...
- [ ] **不带尾斜杠的裸主机基址常量**:代码常写 `const B="https://cdn.example.com"`, `window.shopUrl='https://site.com'` 再拼路径; 只匹配"带尾斜杠"形式的提取/改写规则对它天然漏检. 同类还有 JSON 转义的协议相对写法 `\/\/host\/`.做法: 提取与改写规则覆盖**裸主机 / 带尾斜杠 / 协议相对 / JSON 转义**四种形态, 且**探针要报完整 URL 而不只是 host 直方图**,否则看不出漏的到底是哪一条[objectarchive]
...
核对方式: 每项要么"已补录(见 manifest 行)",要么"确认源站不存在此类",不许留空.
...
-  **一个 200 不是"你拿到了那个资源"的证据**:反爬挑战页 / 同意墙 / 地区拦截页 / catch-all 补充检查页**全部是 200 + `text/html`**,账本会诚实地记下它们的 sha256,而单射性, 闭包, 覆盖度**每一项都在校验一份错误的内容**.**这条坑必须以检查的形态存在, 不能只是这里的一行提醒**--四个项目里它一直只是散文, 代价是整个逆向工作所依据的文档被换掉而无人反对. 可执行形态见 §5.1 的"真实性"断言(`scripts/verify-mirror.mjs` 的 AUTHENTICITY 检查)[objectarchive](实证:`case-studies/mirroring.md` §5.1).
- **catch-all 假 200**:请求 `.map`/任意路径返回 index.html(other-side-of-truth)--对每个下载物做 content-type 校验与哈希碰撞检测(大量文件同 hash = catch-all 补充检查页)[probe].**校验的方向要对**:拿**源站声明的 content-type** 与正文魔数对照, 不要拿 URL 扩展名当预期(扩展名是源站的命名选择, 实测会报误报)[objectarchive].
- **爬虫把挑战页当成功响应落盘**:检查是事后的补救, 爬虫侧的正解是**把挑战页正文当可重试状态**--退避重试, 绝不落盘, 正则只认挑战页独有的标记. 落盘之后它就成了一份"源站从未在那个 URL 上返回过的文件",与 §2 的 `redirect: follow` 造假文件是同一类东西[objectarchive].
- **检查的输入短一截, 和检查的判据错, 是同一族失效, 而前者更难看见**:发现正则少认一种拼法, 豁免按前缀匹配, "什么算文本"是一张扩展名白名单--同一轮里撞到三层.**审一道检查, 先问它的输入是怎么被界定的, 再问它的断言对不对**[objectarchive].
- **范围与清单不一致**: 排除资源后仍沿用完整覆盖的验收结论, 会掩盖缺失. 分别报告访问限制、主动范围排除和技术失败, 见 `case-studies/mirroring.md` §9[objectarchive].
- **零 404 检查在错镜像上通过**:下游每一道检查测的都是"渲染得出来吗",不是"字节对不对".查询参数化资产坍缩成一个文件后, serve 端每个尺寸都回同一份文件, 页面照常渲染, 四道验收检查全部通过--**镜像必须有属于自己的检查**(§5.1)[objectarchive].
...
- **镜像跑不通就开工**:镜像没过 M0.5 检查就逆向/移植, 等于没有对比基准, 后续一切"像不像"都无法归因[lando][samsy].
...
- **直接改磁盘镜像**:一切适配走服务层; 确实不得已改磁盘必须逐处登记并在对比时扣除(rogier 一代约束)[rogier].

## 10. M0/M0.5 验收条件(产出物清单)
...
- [ ] **镜像自检检查通过**(§5.1,跑在断网检查之前):映射单射性 / 账本与磁盘 sha256 一致 / 闭包完整 / **真实性(挑战页正文 + 声明类型对魔数; 体量离群线索逐条读过)** /(联网可选)抽样回源核对
- [ ] `scripts/serve.mjs` 可提供服务镜像, 服务层改写清单逐项登记
- [ ] 断网验收全部通过: 零 404 / 零控制台错误(probe CLEAN,含全滚动)/ 零外联 / 重定向状态码断言通过
- [ ] 外部依赖决策表: 每条外部 URL 有归属决策(保留引用/换端点/接受降级)
- [ ] 镜像盲区 checklist 逐项核对
- [ ] 版权**取证**已出(哪些资产不可再分发, 第三方权利人是谁--事实与建议,**不是 agent 的决断**;"是否公开部署"到交付时交用户决定, 详见 `references/legal-and-deploy.md`)
- [ ] **法务考量未削减镜像完整性**:未采集清单里每一条都是技术性理由(`NOTFILE` / `NOTFETCHED` / `DISALLOWED`),零条"出于版权考虑不抓"[objectarchive]
- [ ] **镜像的存档策略已定并写明**(见 §11):账本(含 sha256)**必须 git 追踪**;`mirror/` 本体是否入库交用户决定, 不入库时产出文档须写明它的存放处与再验证命令

全部勾完 → M0 验收, 进入 M1 逆向(`references/reverse-engineering.md`).

## 11. 镜像的存档策略: 账本必须入库, 本体交用户裁量

`mirror/` 本体是否 git 追踪, 是一个**大小 vs 可追溯性**的取舍, 没有普适答案--几十 MB 的站直接入库;
一个 864 MB 的 Strapi 桶入库会把仓库变成不可 clone 的东西. 这是**用户的决定**,M0 验收时问一次.
但两条不随裁量浮动:
...
-  **账本永远入库**:`mirror-manifest.json`(逐文件 sha256 + 字节 + content-type), `redirects.tsv`,
  ...
  摘要可用于检查候选副本是否与已记录内容一致; 它不能恢复丢失内容.
-  **不入库 ≠ 不存在**:"镜像只在本机"是单点故障, 而获奖站年消失率约 29%--源站死了, 本机盘一坏,
 ...
 一个实测可行的折中[hashgraphvc]:交付所需的子集以**字节固定**的形式入库(逐文件 sha256 清单 + 构建时
逐字节复核后才物化),完整镜像留本地--仓库保持可 clone,交付物自证完整, 全量证据另行存放并登记去处.

## 12. 通用校验入口的使用边界

**用途**:`scripts/verify-ledger.mjs` 反向检查坐标台账. 它按文档声明的相对路径或规范化绝对路径读取源码, 并在同一行核对可选 Needle;不生成坐标, 也不替调用方决定文件归属.

 **解析约束**:同一行出现多个源码路径时, 只有紧邻坐标的路径会被采用; 无法唯一关联时报告歧义, 不按最近文件或 basename 猜测. 路径放在坐标之后时, Needle 应放在另一个括号中, 避免被当成源码路径.

路径错误, 重复映射, 越界行号和不成对的 Dead Zone 标记直接失败. 没有源码坐标的叙述性行保留为未解析报告, 是否升格为阻塞由调用方决定; 校验器输出不反写进镜像.

### 12.1 输入契约与严格程度

最小配置只声明台账文档和源码根; 需要处理同名文件或历史坐标时, 再显式列出源码映射和 Dead Zone:

```json
{
  "prettyRoot": "mirror/_pretty",
  "documents": [
    { "path": "docs/engine-notes.md", "deadZones": [] }
  ],
  "sourceFiles": [
    { "path": "bundle.js", "aliases": ["main.js"] }
  ]
}
```

**严格程度**:`--fatal-unresolved` 只改变未绑定 `L####` 的处置, 不会把叙述性坐标自动变成源码事实. 宽松模式适合先清点文档噪声; 进入阶段前置检查前显式启用严格模式, 并由调用方确认 `checked` 覆盖量符合预期. 工具不设最低覆盖量,`checked=0` 不能解释为"全量验证通过".

**输出**:`--format json` 输出 `ok`, `stats` 和逐条 `findings`;文本输出只供人读. 外层检查根据 `failed`, 严格模式下的 `unresolved` 和预期的 `checked` 覆盖量判定, 不从文本摘要推断全量覆盖.

**路径与时机**:配置中的 `root`, `prettyRoot`, 文档路径和源码路径均按调用时的工作目录解析, 配置文件所在目录不是隐含根目录. 源码格式化, 重新生成或移动 `_pretty/` 后, 先更新并复核坐标系, 再运行本检查; 本检查适合在 M0 验收和每次坐标系变更后使用, 不生成或修复坐标.

**与提供服务层的边界**:独立提供服务是镜像之外的交付适配层. 若交付侧需要资产映射或路由外壳, 应把 `pathname + search`, Locale Cookie, 默认 Locale 和回退规则写入配置;`source-not-provided` 只登记负证据并返回明确 404,不创建占位文件. 资产闭合仍须由引用, 磁盘和负证据三方的检查单独证明(见 `asset-management.md` §8).
