...
- [ ] **映射单射性**：把账本里全部 URL 过一遍 url→本地路径的映射函数，**任何两个不同 URL 落到同一路径即红**。这一项直接抓查询参数化资产；修法是让映射**查询感知**（如 `x.jpg?v=1&width=600` → `x@@v=1&width=600.jpg`），并且**镜像端、serve 端、闭包校验三方共用同一个映射实现**（写成一个模块，不许各写一份——三份实现分歧本身就是新的静默错源）。
