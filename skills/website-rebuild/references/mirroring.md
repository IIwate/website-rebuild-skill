...
⛔ **解析纪律**：同一行出现多个源码路径时，只有紧邻坐标的路径会被采用；无法唯一关联时报告歧义，不按最近文件或 basename 猜测。路径放在坐标之后时，Needle 应放在另一个括号中，避免被当成源码路径。
...
**严格程度**：`--fatal-unresolved` 只改变未绑定 `L####` 的处置，不会把叙述性坐标自动变成源码事实。宽松模式适合先清点文档噪声；进入阻塞门前显式启用严格模式，并由调用方确认 `checked` 覆盖量符合预期。工具不设最低覆盖量，`checked=0` 不能解释为“全量验证通过”。

**输出**：`--format json` 输出 `ok`、`stats` 和逐条 `findings`；文本输出只供人读。外层门根据 `failed`、严格模式下的 `unresolved` 和预期的 `checked` 覆盖量判定，不从文本摘要推断全量覆盖。

**路径与时机**：配置中的 `root`、`prettyRoot`、文档路径和源码路径均按调用时的工作目录解析，配置文件所在目录不是隐含根目录。源码格式化、重新生成或移动 `_pretty/` 后，先更新并复核坐标系，再运行本门；本门适合在 M0 关账和每次坐标系变更后使用，不生成或修复坐标。

**与伺服层的边界**：独立伺服是镜像之外的交付适配层。若交付侧需要资产映射或路由外壳，应把 `pathname + search`、Locale Cookie、默认 Locale 和回退规则写入配置；`source-not-provided` 只登记负证据并返回明确 404，不创建占位文件。资产闭合仍须由引用、磁盘和负证据三方的门单独证明（见 `asset-management.md` §8）。
