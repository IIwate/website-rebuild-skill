...
选型判据有两条,**按序问**:先问"DOM 被谁消费"(决定字节检查的性质与选型自由度),再问"DOM 由谁生成"(决定 A/B/C).两条都在镜像 HTML / bundle 里取证.
...
- **策略 D 特征**:同一个函数里同时出现 `querySelectorAll("[data-*]")` + `getBoundingClientRect()` + `getComputedStyle()` 三件套[shopifydesign].命中后**必须再做运行时取证**:hydration 前后同一批节点的 `getBoundingClientRect()` 是否变化(§5.2 2))--SSR DOM 常常只是场景图的初值.
- 分支可组合: lando 是"平台外壳(策略 A)+ 自定义 bundle 应用层手写重写"的混合--外壳与应用层可分别选策略[lando].

**共同验收(三策略通用)**:产出 HTML 与镜像做"空白归一化 diff 为空"或逐字节 diff 为空[oryzo][noomo][kimi].字节层的检查要**最先建立, 终身保持全部通过**--"字节层先行使后续所有视觉 debug 都能排除 DOM/payload 差异"[noomo].
...
| **D DOM 即场景图**(正交约束) | DOM/CSS 被 3D 引擎当**坐标源**读取 | 同 A(约束一旦命中, A 是唯一正确解; 但 A 只保证初值, 见 §5.2 2)) | 场景图**逐字段数值全等**(几何检查, 基准取**运行时静止态**),字节检查是它的前提 | [shopifydesign] |
...
1. 写 `gen-shells.mjs` 类生成脚本, 对镜像 HTML **只做登记在案的变换**.lando 全部只有 4 项:
   - 1) 剥离遥测脚本(登记为偏差: 私有部署不应上报);
   - 2) 外部 host URL 重写为 `/ext/<host>/` 本地路径(登记为偏差);
   - 3) 把源站 bundle 的 `<script>` 标签替换为自己的模块入口(`<script type="module" src="/src/app/main.ts">`)--这一处替换就是"重建本体";
   - 4) 仅当 parser 无法解析时做最小修复(lando 修一处畸形 SVG 属性边界让 parse5 能解析, 浏览器 DOM 等价, 登记为偏差).
2. **其余一切逐字保留**--包括注释掉的历史脚本块(登记为源站特殊行为 Q1)[lando].
3. **脚本内置"变换没发生就 throw"的防御--按逐条下限写, 不按总数写**[lando][objectarchive].这条约束有两个强度,**默认用强形式**:
   ...
   - **强形式(默认)**:**每条登记变换单独声明期望命中次数或下限, 构建时逐条校验; 任一条为 0 或低于下限即 throw**.下限按**固定的镜像快照**量出来(快照固定表见 `reverse-engineering.md` §0.1.1),所以重抓镜像改了 markup 形态同样会立刻明确报错并非零退出.
   - 总命中数大于零不能证明每条变换执行. 高频 URL 本地化可掩盖其他必需变换的缺失, 见 `case-studies/dom-shell-strategies.md` §2[objectarchive].
   - 命中数下降也可能来自输入文档变化. objectarchive 的挑战页替换触发了这一检查, 但响应真实性仍需独立核对, 见 `mirroring.md` §5.1.
   ...
   -  **这条论证与"变换"无关, 它是关于"表"这个东西本身的**--凡是**记录状态的表**(变换表 / 分层归属表 / 核对·进度表 / §6 偏差表 / §Q 源站特殊行为表)都要有一道从产物或运行时**反查它**的检查. 通用条款, 双向判据, 以及"**共用宿主 ≠ 覆盖**"的实证(核对表建起来第一天就抓到一条虚报)见 `porting-discipline.md` §4.1[objectarchive].
4. 配套路由/资产层(lando 的 vite 两个自定义插件):
   - `extAssets()`: 开发时将 `/ext/<host>/` 映射到 `mirror/assets/`, 避免重复存储大型资产; 自包含交付另按资源清单准备.
...
- [ ] shim 清单中每条都有取代计划, 交付时清零.

## 4. 策略 C:框架内重建 + 字节对齐(框架编译产物)

适用: DOM 由框架运行时/SSR 生成, 无法"直接搬 HTML",必须在同栈同版本框架内重建, 然后**用字节对齐检查证明重建输出与源站编译产物等价**.按框架分四条子路线:
...
- DOM/应用层 1:1 覆盖(samsy:13 组件, router 守卫源站特殊行为照抄),文本细节到码点: 自研 SplitText 移植时不可见字符逐码点核对(U+200B/U+00A0/U+202F)[samsy].
...
- 建立可重复执行的检查 `verify-ssr.mjs`:9 路由 body/payload/config 与镜像逐字节 diff + 尾部脚本顺序 + 未知 slug 404 行为,**每 commit 必跑**(noomo 的 commit message 几乎每条以 "SSR gates green" 结尾)[noomo].
- Pinia store 全签名移植(30 state + 29 getters + 33 actions,**死代码照抄**)--payload 字节对齐会暴露任何字段缺漏[noomo].
- **传递依赖也要固定**:同一 Nuxt 版本不等于同一输出--unhead 2.0.17 vs 2.1.17 会反转 bodyClose 脚本顺序, 破坏尾部字节序, 用 overrides 固定[noomo].
...
- 服务端行为在客户端产物里零留痕: redirects 必须逐 URL 实测状态码; Next `permanent: true` 发 308 而源站发 301--**检查必须断言状态码本身**,差异登记为偏差[kimi].
- 契约检查覆盖面(kimi `verify-routes.mjs`,81 项经审查扩到 94 项):head 8 字段 × 5 路由, 12+8 条重定向含状态码与尾斜杠链, 源站特殊行为可达性, assetPrefix, favicon[kimi].

### 4.4 CSS 层: 双向 diff[rogier]
框架内重建时 CSS 无法整体照搬的, 用双向 diff 完成验收:
...
## 4.5 环境条件控制分支: localhost 语义分叉

源站发布产物里常内联**按 host 判定环境**的分支, 最典型是主题/框架的 dev 开发分支:

```js
if (location.hostname === 'localhost') { /* Probe the Vite dev server and connect HMR. */ }
else { /* Production path. */ }
```

当复刻站使用 `localhost` 时, 可能进入源站正式域名下未执行的开发分支, 产生 dev 端口探测请求. 这类分支需要通过条件与调用点确认, 不能只凭平台名称推断[racingshop].

两条路线, 按项目目标选,**都必须登记**:

| 路线 | 做法 | 代价 | 何时选 |
|---|---|---|---|
| **保持 verbatim**(默认) | 一字不改, 把分叉登记进 §Q 源站特殊行为表 | 本地跑会有 dev 探测噪声; 需在 CLEAN 检查白名单里放行并写明理由 | 追求字节级忠实; 噪声无外联, 无副作用(racingshop 选此, 登记为 Q1)[racingshop] |
| **强制生产分支** | 改写条件使其恒走 else 分支 | 属于**自创改动**--违反"源站有的都要有"的字面约束, 必须登记进 §6 偏差表并说明"何时重新考虑" | 噪声会污染验收检查信噪比, 或探测行为有真实副作用(外联/报错/阻塞渲染) |

判定顺序: 先看这条分支**有没有副作用**(外联? 抛错? 阻塞?).无副作用 → 一律 verbatim + 源站特殊行为登记, 这是约束的默认答案. 有副作用 → 才动它, 且按偏差登记, 不要顺手"清理干净".
...
**§5.1–§5.3 讲的是完整形态(站级, 命中即锁死外壳选型);§5.4 是它的弱化形态**--站上没有任何 3D 引擎, 只有个别块在运行时量矩形, 写内联样式: 同族, 块级, **不锁死选型**,但那几个块的检查必须按几何量断言[objectarchive].
...
**1) 静态取证: 三件套.** 在 bundle 里搜:**`querySelectorAll("[data-*]")` + `getBoundingClientRect()` + `getComputedStyle()` 同时出现在同一个函数里**--命中即按策略 D 处理.(三者单独出现不算数: 测滚动位置, 判响应式断点都会用到前两个.)

**2) 运行时取证: hydration 后布局是否被改写?--SSR DOM ≠ 场景图.**

在镜像上用**同一份场景解析探针**采两次, 比对同一批节点的 `getBoundingClientRect()`:1) **纯 SSR 排版**(摘掉框架运行时, 或在 hydration 接管前量);2) **hydration 后的静止态**(框架 effect 跑完, 所有异步回填结束).两次有差 → 服务端下发的 HTML 只是场景图的**初值**,改写后的结果才是引擎读到的坐标. 纵向功能切片期会自然撞上这个形态: 把 SSR 外壳原样端起来, 只换引擎, 数值检查第一次跑就红.(实证:`case-studies/dom-shell-strategies.md` §5.2)

**三条后果(判据命中后立即生效)**:

1. **镜像 HTML 的 DOM 顺序不能当作场景顺序的规格.** 规格在**重排代码**里(那段砌砖/定位函数),SSR 结果只是它某一次的输出. 把 SSR 顺序抄成固定表 = 把一个中间态固定成规格, 等真移植了重排层, 这张表要么删掉要么变成掩盖 bug 的补偿层.
2. **对比基准必须取自运行时, 不是静态 HTML.** 采基准要等到重排的输入齐了(本站 = 最后一次 `onLoadedMetadata` 回填, 静止态达成)再抓; 镜像侧与复刻侧都按同一个"静止态判据"抓, 不按墙钟等待时长.
3. **框架布局层从"某个里程碑的一个模块"升格为场景正确性的前置依赖**,排期必须提前--不移植它, 数值检查在原理上就不可能通过(shopifydesign M2 因此延后 102 个字段, M3 移植布局层后全部归零).这也说明**策略 A 是必要条件而非充分条件**:零重写外壳只保证初值逐字正确, 不保证场景正确.

### 5.3 三条推论(每条都改变工程决策)

1. **字节检查升格为几何检查.** 现有三策略把 DOM 层当"外壳",字节检查是**文档保真**的检查; 策略 D 站上 **CSS 差 1px,3D 物体就位移 1px × 全局缩放**,字节检查变成 **3D 正确性**的检查. 于是策略 A(零重写 shells)从"可选的省事做法"升级为**唯一正确做法**--任何重写, 切分, 框架内重建都是在往坐标源里注入误差, 且误差会以"3D 位置不对"的形态显现, 不会被当成 DOM 问题去查.(同时注意 §5.2 2):A 只保证 SSR 初值逐字正确, 若 hydration 后布局被改写, 还得把那段重排代码也逐字移植, 几何检查才可能通过.)
...
3. **它顺带带来一个比像素检查更该先建的检查**:把引擎读 DOM 的那个函数逐字转写成探针, 两侧逐字段比数值. 判据与建检查方法见 `references/verification-gates.md`.
...
**1) 识别判据**(三步齐备即命中; 与 §5.2 1) 的三件套区别在于**没有 3D 引擎参与**):
...
- **先在矩形上收敛, 再采样**:等被量的那个盒子不再变,**不要去等块自己的防抖时钟**(实证里的合成器有 120 ms 防抖, 检查等的是矩形, 全程无墙钟);
- **实测量级**:见 `case-studies/dom-shell-strategies.md` §5.4;
-  **几何类实证必须连同视口, DPR, 测量判定标准一起记, 否则它既不可复现也不可证伪**[objectarchive].判定标准三选一, 同一个盒子可以给出**三个不同的数**:
  - **内联样式值**(`el.style.height`)--**块自己写下的产物**,是这类检查该断的东西;
  - **计算值**(`getComputedStyle`)--可能被别处的 CSS 覆盖掉, 断它等于把主题 CSS 也算进被测块;
  - **客户端矩形**(`getBoundingClientRect`)--含 transform 与亚像素,`221` 与 `221.33` 是两个判定标准而不是两次测量误差.
  - **推论(全库通用)**:既有实证里凡是只有像素数, 没有视口/DPR/判定标准的, 都只能当**形态证据**读("这类量会差, 会显形"),**不许当期望值抄**,引用前必须在自己的视口下重测. 已知同类: 本文件 §3 的 `scrollHeight 46410px`, §5.2 实证的 `.hero-grid 4078 vs 4175px` 与 `docHeight 13798 vs 13895`, §5.3 推论 2 与 `verification-gates.md` §1.4.1 的"统一 158px Z 偏移".
- **记录约束**:过渡 / 动画途中量到的高度**不进比对产物**--它属于"这一次运行",记派生判定(`gate-failure-modes.md` §1.11).

**3) 与完整策略 D 的关系(同族, 弱化, 两者不许相互冒充)**:

| | 完整策略 D(§5.1–§5.3) | 本节的弱化形态 |
|---|---|---|
| 谁在读 DOM | 3D 引擎, 把全站排版结果读成**世界坐标** | 站点自研的**单个块**,只读自己宿主的矩形 |
| 出错后果 | 3D 物体位移, 排查方向天然跑偏 | 这一个块的**缩放 / 高度**不对, 作用域限于宿主 |
| 对外壳选型的影响 | **锁死策略 A**,字节检查升格为几何检查 | **不锁死选型**--外壳照 §1 决策树选; 但**字节检查仍是几何正确性的前提**(宿主的 CSS 差 1px 就够了) |
| 该建什么检查 | 场景图数值检查(`verification-gates.md` §1.4.1),全站一道 | **块级几何检查**,逐块一条, 挂在该块的运行时检查里 |
...
4. **构建器会悄悄改字节**:vite 对 srcset 内 URL 二次编码 `%20→%2520` 导致 7 张含空格文件名的图 404,自动检查抓不到, 靠目视补充检查 + postbuild 还原 + 登记偏差; 教训:"srcset/style 内 URL 的编码保真需要纳入构建期对比"[lando].
...
8. **策略 C 忘记钉传递依赖**:框架小版本, 传递依赖都会改变输出字节序, 字节检查失败时先查依赖树再查代码[noomo].
9. **生成结果检查不足**: 仅检查变换总数非零, 可能遗漏单条变换失效. objectarchive 的 2,540 次 URL 本地化可掩盖原本 5 次的 noindex 注入缺失. 对必需变换分别设置条件, 并检查生成字节, 见 §2[lando][objectarchive].
   ...
   >
   > **一般形式**:凡是"移除/替换/注入"类变换, 下限与**目的断言**要成对出现. 下限回答"它还有靶子吗",目的断言回答"靶子被打掉了吗".用一次性临时输入人为破坏一条规则, 两条应该**各报一次红**;不要求维护持久测试资产.
...
11. **把 SSR HTML 的 DOM 顺序当成场景顺序的规格**:hydration 后若有客户端重排, SSR 结果只是初值; 照它固定顺序会在重排层落地时反转成补偿层, 而对比基准取静态 HTML 则一开始就量错了对象. 判据与取证方法见 §5.2 2)[shopifydesign].
12. **"站上没有 3D,所以几何不用管"**:站级判据不命中策略 D,不代表没有块在运行时量矩形, 写内联样式--这类块的检查若只断类名或样式字符串,1px 的 CSS 差会一路绿着穿过去, 显形为缩放/高度不对. 判据与建检查方法见 §5.4[objectarchive].
13. **几何实证记成裸数字**:只写像素数, 不写视口 / DPR / 测量判定标准(内联样式值 · 计算值 · 客户端矩形),下一轮复现不出来也证伪不了--同一个盒子在 390×844 下内联 179px, 计算值 650px,都是"对"的. 记法要求与实证见 §5.4 2)[objectarchive].

## 7. 静态外壳提取的边界(SSR 取样)

**适用条件**:需要从一次 SSR 响应提取可复用外壳时. 入口, 路由元数据, 状态岛选择器, 输出目录和待删除脚本都是**调用方事实**,应由配置或记录表声明; 模板只提供字段形状.

 **提取约束**:只删除明确列出的旧脚本, 其他脚本的属性, 顺序, nonce 和 SRI 原样保留. 提取后至少反向检查完整 DOM, 状态岛和路由元数据; 路径越界, 目标脚本缺失或重复命中都应明确报错并非零退出.

静态 HTML 相等只证明外壳字节, 不能替代 hydration 后的状态和几何验收. 这里规定提取协议和失败边界, 不提供跨框架生成器.

- [ ] 外壳输入和输出的身份, 来源坐标及复现命令已记录(资产取证见 `asset-management.md` §8);
- [ ] 运行时缺失资源已登记为 `source-not-provided`,提供服务层返回明确 404;
- [ ] 外壳字节检查与运行时几何/状态检查分开, 未用静态 HTML 冒充 hydration 后的行为证据.
