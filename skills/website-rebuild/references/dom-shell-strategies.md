...
⛔ **提取纪律**：只删除明确列出的旧脚本，其他脚本的属性、顺序、nonce 和 SRI 原样保留。提取后至少反向检查完整 DOM、状态岛和路由元数据；路径越界、目标脚本缺失或重复命中都应响亮失败。

静态 HTML 相等只证明外壳字节，不能替代 hydration 后的状态和几何验收。这里规定提取协议和失败边界，不提供跨框架生成器。

- [ ] 外壳输入和输出的身份、来源坐标及复现命令已记录（资产取证见 `asset-management.md` §8）；
- [ ] 运行时缺失资源已登记为 `source-not-provided`，伺服层返回明确 404；
- [ ] 外壳字节门与运行时几何/状态门分开，未用静态 HTML 冒充 hydration 后的行为证据。
