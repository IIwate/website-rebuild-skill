...
3. **为每个即将冻结的源列出它下游的入口**（从它里面派发的事件、resolve 的 promise、翻转的就绪标志，以及这些信号的消费者），逐个决定"探针泵到 / 补不冻结抽查"——**冻结会让挂在被冻分支上的子系统对门隐身**，这一步是冻结的配套纪律而不是可选建议（§2.10）【shopifydesign】；
4. 双侧同协议注入（同一份补丁代码打在镜像与复刻两侧）；
5. 跑 §4 防呆断言，确认冻结与驱动都真的生效了。
...
1. **列出所有挂在被冻熵源上的入口。** 对每个要冻的源（rAF / timer / clock / random / visibility）做两跳 grep：
...
3. **清单入库**：被冻源 → 下游入口 → 处置（泵到 / 不冻结抽查 / 明确不需要）逐条写进本站熵源清单（§8），与 shim 覆盖面验收记录同一本账。**冻结项每加一条，就得补一条处置**——这是"冻得越狠盲区越大"的收费口。
4. **推论：不冻结的对拍不可省。** 数值门与像素门不是替代关系，数值门原理上看不见被冻住的分支；每个里程碑至少保留一条不冻结的截图对拍或结构性抽查（见 `references/verification-gates.md` §1.4.1、§4.7）。

**新子系统落地时复查**：移植进来的子系统若挂在 `site-ready` / rAF / 定时器之后，它一进来就落在盲区里——建门时先回到本节第 1 步重新枚举，别假设上一轮的入口清单还是全的。
...
- **被冻源的下游入口清单**：被冻源 → 挂在其上的事件/promise/就绪标志 → 消费它的子系统（带源行号）→ 处置（泵到 / 不冻结抽查 / 不需要），每次新增冻结项或新移植子系统落地时复查【shopifydesign】
