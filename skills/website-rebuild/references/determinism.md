...
因此要在任何应用与库对象构造前注入种子，钉死依赖版本，并按源站物理执行序记录关键构造器及累计随机调用数。可在启动阶段临时给 `Math.random` 加计数探针，按区间对拍消费预算；完成取证后不把诊断探针留进生产路径。⛔ 只核对显式业务调用，不能证明两侧随机流同位；消费次数或顺序无法稳定时，随机分支应拆开强制验收，不能宣称字节确定性。
...
- [ ] 随机数消费预算与关键构造顺序已按源站记录并核对，依赖版本已钉死【ilithya】
