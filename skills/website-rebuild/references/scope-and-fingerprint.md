...
> **Node 脚本入口**: 用 [`scripts/fingerprint.mjs`](../scripts/fingerprint.mjs) 一条命令等价执行本节六步: `node <skill>/scripts/fingerprint.mjs --target "<TARGET>" [--bundle <bundle-url>]`. 产物落在 `probe/` (a.html / b.html / bundle-*.js / fingerprint-report.md), 计数采用出现次数语义, 下载物逐个记 sha256. 脚本**只采证据不出判级**, §3 判定树, §4 三判据与 §6 的解读仍须人工执行.
