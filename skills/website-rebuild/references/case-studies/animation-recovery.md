...
- 事件的**触发者, 条件控制条件, 先后序**都是规格; preloader 最短展示时长这类检查槛值(samsy 的 4000ms)也要抄[samsy].
...
3. 交付期排查弱视觉差 F3 时, 拿 dev.json 采样值逐项核对 8 个现场弹簧值, 证明"参数绑定链无 bug"后才定性为已知差异登记[noomo].
...
5. kimi 曾只采 `<main>` 上 18 个变量, 在位置 3.2 后"完全漏检"(变量饱和, 场景 3-7 由容器 opacity 驱动)--把 opacity 采进基准后覆盖立刻到 8.2[kimi].
...
1. **魔数逐字照抄**:`wheelEaseCoeff=12`[oryzo];Lenis 配置逐字 `{lerp:0.1, touchMultiplier:1.25, syncTouch:true…}`,连"两分支配置相同"的源站特殊行为(Q7)也照抄[lando].
...
3. **"好心修正"非惯常写法**:带符号取模被修成正取模后 About 页浮动方块全部消失[rogier];lando "修好" `scene.remove(Q.name)` 的 no-op 后真删除反而破坏遍历导致转场崩溃, 最终回抄(Q13)[lando].
...
6. **基准录制的覆盖盲区**:只录部分变量/部分驱动域会"漏检"(kimi 位置 3.2 后饱和)[kimi];录完基准先验证覆盖度.
9. **自创补偿性动画/CSS**:JS 机制没对齐时用自创 CSS 补观感, 等 JS 对齐后补丁反转成 bug(rogier 十余个视觉 bug 的共同根源)--"宁可先不像, 也未启用功能规则"[rogier].
