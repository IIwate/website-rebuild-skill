...
- 按 `scrubber=27` 把签名行为定位到一个 85 行的模块, 写进了纵向功能切片计划;
...
实测在一个无容器的 vendor chunk 上: 覆盖 7%, 239 处 require 调用对 0 条边 → 正确 FATAL(exit 5).
 ...
 webpack 与 Turbopack 探测应分别完成, 再汇总容器结果. 该实现曾在 webpack 分支未找到容器时直接退出, 导致第 1 行包含 Turbopack 容器的 chunk 被漏判. 其他 chunk 中两处无关的 `key: function` 恰好阻止了提前退出, 掩盖了控制流缺陷.

 只取紧挨工厂的那一个, 会**静默丢掉其余全部**.实测一个站的 5 个 chunk 里有 **10 个别名 id** 被丢; 症状是运行时抛 `module 73692 … the module factory is not available`,**而那个 chunk 切片是字节一致的, 静态检查全部通过**--因为丢的不是字节, 是**注册表里的键**.
...
**跨路由内容核对**: Lenis+GSAP 块 `B:41e7f747ed2a` 分别位于首页 L9564-9687、collection L8468-8591、product L11658-11781, 内容摘要相同. 原本按行号记录的 9 个条目可按内容归并为 3 个, 并保留各路由坐标.
...
- 版本沿革: samsy 首次把版本固定制度明文化(当时 2.0.3),kimi/noomo/lando 三代统一 1.15.1--本 skill 钉 **1.15.1**,不要用别的版本[samsy][kimi][noomo][lando].
...
**为什么必须前置**:objectandarchive 在 Step 0 预登记, M1 开头证伪, 于是**在写第一行移植代码之前**就换掉了坐标方案, 代价是半天.**同样的发现若拖到 M2 中途才撞上, 笔记, 移植文件头注释, 里程碑待办, 源站特殊行为/偏差表里的坐标引用早已铺开(前作规模 107–400+ 处),一次性全部作废且无法自动修复**--与"beautifier 版本漂移"是同一类灾难(§1 ),只是触发源不同.
...
**第二段: 源站特殊行为清单(照抄不修)**:源站 bug / 死代码 / 非惯常写法逐条登记并带坐标(行号或 `B:`),移植时逐字照抄. 规模参考: noomo Q1–Q14, samsy 13 条, kimi 26 条[noomo][samsy][kimi].

**第三段: 对复刻的直接结论**:如 noomo 的 10 条("先实现三个元系统再写任何材质", "缺 colorsMap 玻璃会变灰白")[noomo];samsy 的"未启用功能"清单(engine-notes §16)[samsy].
...
grep 命中只是假设,**每条必须回上下文确认**;**计数同理**--`grep -c` 数的是匹配行数不是出现次数, 且 vendor 自带字符串(报错串, 内置 shader chunk)会把应用层用量抬高一个数量级(shopify.design:`ScrollTrigger ×8` 实为 0 次真实使用, `GLSL ×107` 实为应用层 27 段).这条约束已前移复述到 Step 0,见 `references/scope-and-fingerprint.md` §2《计数硬约束》[shopifydesign].逐条实例: kimi 站 grep 到 `leva` 实为 React SVG 属性列表里 `…decelerate|descent…` 的子串误命中,`swr` 同类;`zustand` 反而真实存在只是被内联[kimi].samsy 早期指纹误判"有 GPU compute",M1 证伪--`dispatchWorkgroups` 字符串全部来自 three 内部; KTX2/meshopt 能力在 GLTFLoader 里但从未挂载[samsy].

### 4.2 核实架构假设

规则见 `reverse-engineering.md` §4.2.

kimi 的依赖表包含 three.js 和 R3F, 但主要视觉内容由 DOM、18 个 CSS 自定义属性、`clip-path` 及三个 2D Canvas 软件渲染器实现. 两个 `<Canvas>` 均为懒加载, 3D 场景为正交相机与 32 个 plane, 未发现自定义着色器. 依赖列表不足以代表实际渲染架构[kimi].
...
**基准覆盖面判据**:录之前先确认"观感由哪些量驱动",把全部驱动量采进基准--kimi 只采 `<main>` 上 18 个变量, 位置 3.2 之后变量饱和, 场景 3-7 实由容器 opacity 驱动, 基准"完全漏检";补采 opacity 后覆盖立刻到 8.2[kimi].
