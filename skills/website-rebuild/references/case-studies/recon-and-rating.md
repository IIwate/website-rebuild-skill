...
**判例(本条约束的来源)**:careers.kimi.com 的依赖表里有 three.js + @react-three/fiber,按直觉会判"WebGL 站"并把主力投进 3D 管线. 逆向确认的事实是: 视觉主体是 DOM + 18 个 CSS 自定义属性 + `clip-path` 擦除 + 三个 2D canvas 软件像素渲染器;`<Canvas>` 只有两个且都懒加载, 唯一真 3D 是正交相机 + 32 个 plane, 零着色器. README 把"误判架构"列为头号风险[kimi].
...
|---|---|---|---|---|
| "这是 WebGL 站" | 依赖表含 three/r3f | 数 Canvas 挂载条件, shader 数, 视觉主体驱动层 | `<Canvas>` ×2 皆懒加载, 零着色器 | **证伪**:DOM+CSS 变量站[kimi] |
| "有 GPU compute" | grep 命中 dispatchWorkgroups | 命中回溯 bundle 区段归属 | 全部落在 three vendor 区段 | **证伪**:应用层零调用[samsy] |
...
2. **误命中的反向也存在**:`zustand` 确实在用, 只是被 r3f 内联--grep 命不中不代表不存在(API 指纹反而可确认:`getInitialState` 无 `destroy` ⇒ zustand v5)[kimi].
...
技术栈版本的确认标准(六项目一致[6/6]):版本字符串(`versions:{get nuxt(){return"4.2.1"}`[noomo], `window.next={version:"16.1.6"}`[kimi]), pnpm 路径泄漏(一次固定 next/react/babel/sass 四个版本[kimi]), wasm URL(Rive 版本取证[lando]), API 指纹. 传递依赖必要时用 overrides 固定(unhead 2.0.17 vs 2.1.17 会反转脚本顺序--"同一框架版本不等于同一输出")[noomo].
...
- 每一难度等级写一句"为什么",引用镜像/bundle 证据, 不凭平台名/框架名印象(webflow.com 被预判不适用, 实测有手写 GSAP/three.js bundle 判 A[probe]).
- **素材版权单独评估且经常是最高星**:oryzo 与 kimi 都评 5/5,"最大风险是法务不是技术"[oryzo][kimi];lando 同样"素材版权 5/5 远大于技术",因此**开工就按安全默认执行"私有仓库 + 不公开部署"**并写进 DEPLOY.md[lando].
...
攻坚顺序: 星多的分项先**纵向功能切片一条端到端链路**验证可行性(oryzo 先打通 hero 场景完整链路再铺开[oryzo]).
...
- objectarchive: 未采集项约占补采后文件数的 60%, 五项检查仍全部通过. 需要单独检查发现集合和排除项, 见 `mirroring.md` §5.1[objectarchive].
