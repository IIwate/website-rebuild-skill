...
**步骤 4**--验收标准必须是可数的: oryzo 的检查是 **25/25 模型全部解析成功**[oryzo].
...
- 数据不在磁盘文件里, 而在 **worker 协议**中--把 baker worker 与主 bundle 一样用固定版本的 js-beautify 展开进 `_pretty/`(samsy:33,458 行),协议全量写进 engine-notes 再移植[samsy].
...
1. **运行时资源发现**: oryzo 的 `.sog` WASM 排序 worker 和 samsy 的 baker.worker 未被当时的静态采集发现, 后续通过浏览器请求补录. 能否静态发现取决于引用形式与扫描范围.
2. **数据类资产用脚本抽取, 不手工复制**:
   ...
   - kimi: i18n 通过括号配平和 vm 求值抽取, 键集交叉校验为 80/80. 生成内容保留了源站的拼写错误[kimi]. vm 不构成处理不可信代码的安全边界.
...
7. **自写二进制工具必须对参照实现验证**:
   ...
   - 诊断工具与验收检查要用不同的正确性标准, 一份解码代码同时服务两者时, 坏账会藏在全部通过里[kimi].
