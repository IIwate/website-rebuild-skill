# case-studies/scope-and-fingerprint.md - 第 0 步: 范围判定与指纹路由( 阶段前置检查) 的实证记录
...
- **token 级差异**:仅 nonce/随机装饰串(kprverse 全文只差 12 个字符的装饰性编号; 某些 WAF/CDN 每次注入的轮换 token 同类)→ 仍可镜像, 验收检查加掩码规则,**不要误判为动态渲染判 D**[probe].
- **内容级差异**:文案/结构/数据随请求变(A/B 分桶, 个性化注水)→ D 信号(airbnb)[probe].

### 步骤 3:站点类型/年代校验(防"隐性下线")
...
shopify.design 的 Step 0 用 `tr ';{}' '\n' | grep -c` 数 token,一次产出三个假数字, 且**直接进了难度评级表**--"滚动/动画编排 3/5"整条建立在一个根本没被使用的 ScrollTrigger 上:

| Step 0 报的 | 逆向后的实际 |
|---|---|
| `ScrollTrigger` ×8 | **0 次真实使用**--全是 gsap core 对未注册插件的补充检查钩子, 插件从未 `registerPlugin` |
...
**同一类错误在 `__reactRouterContext` 上重犯过一次**[shopifydesign]:该信号此前被本文件列为 C 信号, 出处是 Shopify Editions spring2026--但那站判 C 的**真因是 R3F + Theatre.js(声明式引擎)**,与 React Router 本身无关. shopify.design 命中同一信号, 逆向后确认为 **A**:47,224 行**命令式** three.js 引擎全在客户端 chunk 里(无 R3F, 无 Theatre),React Router framework 模式**下发 route module**,1.24MB `_index` chunk 就是引擎本体.**信号被记在了错误的维度上**--框架名带来的是"下不下发行为源",引擎模式才决定"下发的东西能不能转写".
...
| 换进页面 | **CLEAN**,8 个 canvas 与镜像一致, 跨侧 **99.5%**(残差 1.18 / 自比带宽 0.68,同量级) |
 ...
 判 C2 之前仍要确认两件事:1) 站点**自有**代码用了那个库(不是 vendor 里躺着);2) 那些回调里确实是数学与状态写入, 不是空壳. 两条都可量化, 如上表.

## 5. 探测约束(14 条协议修正, 逐条为实测教训)[probe][shopifydesign]
