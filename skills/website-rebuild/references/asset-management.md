...
### 6.1 授权网页字体：保留原引用，不入库

Adobe Fonts（Typekit）类商用授权字体**不可自托管**：保留源站的 Typekit CSS 引用、接受离线时的字体回退【oryzo】；镜像抓到的副本存 `legacy-mirror/external/typekit/` 仅供参考，不进运行资产【samsy】。
