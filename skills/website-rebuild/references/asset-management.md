...
**适用条件**：产物拥有独立输出根或服务层时。镜像账本记录源站 URL 空间；产物清单记录实际提供的 `pathname + search`。两者不能靠文件名互相推导，映射策略应写进调用方配置，并在生成后复核。

⛔ **负证据不等于占位文件**：`source-not-provided` 只登记请求、原因、证据和检查时间；伺服层对该项返回可识别的 404。把空文件或猜测性替代品写进产物，会把"没有证据"伪装成"已经闭合"。

本节规定资产集合的边界，不生成统一资产门。调用方选择已有的 manifest/引用门，或按产物清单实现适配门；引用集合、可提供文件集合和 `source-not-provided` 集合必须分开核对。

- [ ] 产物身份、输入清单、输出根目录和复现命令已记录；
- [ ] 第三方资产的来源、归属、许可证据和哈希已登记；
- [ ] 引用集合、磁盘集合和 `source-not-provided` 集合分别列出，并做闭合检查；
- [ ] `pathname + search` 的每个变体均按实际响应验证，未声明的变体不会回退到另一份资源。
