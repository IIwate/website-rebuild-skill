...
前六条来自 kimi 对"门"本身的系统反思，全部有实锤事故【kimi】；第七条由 shopifydesign 补上，讲的是**门的前置条件（确定性冻结）反过来吃掉覆盖面**——它与前六种并列，是新的一类。设计每个门时逐条自检：
...
- **不冻结的结构性抽查记录**（绝对断言而非双侧 diff），与冻结门的产物并列入库——它覆盖的是冻结门的盲区【shopifydesign】
