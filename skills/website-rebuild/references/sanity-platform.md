...
>
> **置信度声明**:本文实证来自五个样本--hashgraphvc(Nuxt 3 + Sanity,L2 完成验收)
> [hashgraphvc], basement.studio(Next + Sanity,已交付)[basement],
...
操作化: 断网提供服务镜像看首屏是否发 `*.api(cdn)?.sanity.io` 请求; 对照 flight/payload
里内容是否已齐. 判定结论连同 projectId/dataset 钉入 `engine-notes.md`.

## 1. 镜像层

### 1.1 CDN 主机范围与 GAP 统计
...
1. 镜像提供服务的字节 ≠ 浏览器在源站拿到的字节(jpeg 压痕 vs webp,体积差实测可达 18×),
   **对源站保真这一维度上是静默偏差**;
2. 两侧都从镜像读时, 跨侧检查, 像素检查**仍通过**--这是"错的镜像能让下游检查全部通过"的又一实例;
...
- 新镜像:`mirror-site.mjs` / `reconcile-gaps.mjs` 对图片 URL 自动发**浏览器同款图片
  Accept** (`lib/negotiate.mjs` 的 `IMG_ACCEPT`, 记录所用 Chrome 请求头;
...
- 存量镜像:**原始镜像只读, 不许原地改字节**--分叉登记为偏差(源站怎么发 / 镜像存了什么 /
  ...
  覆盖检查无需改语义; 以浏览器字节为参照的检查用 `serve --fallback-root` 链 negotiated→mirror.
...
GROQ query 原样保留; 断网下 404 壳会**反复重试同一查询**(源站特殊行为 Q3,照抄行为, 只本地化
...
- `cdn.sanity.io/robots.txt` 按 project 路径逐条声明(hashgraphvc 实测: 允许
  ...
  约束**在第三方 CDN 上同样适用, 不因存在 Disallow 行判全站禁止[hashgraphvc].
...
** 协商面三族**(全部实测; 镜像账本 v0.3.9 起记 `vary`,验收前对账本 Vary 普查一遍即得全景):
...
**verify-flight 常用旗标**(C1 完成验收):`--normalize-props` 收 ISR 纪元字段(Sanity 站
内容漂移的投影,§2);Vercel 动态流 vs 本地静态构建的 row-0 平台字段差由 N11 内建
(v0.3.2). `_key` 参与对象身份判断, 在这组比较中保留其值.

## 5. 验收 checklist(Sanity 场景)
