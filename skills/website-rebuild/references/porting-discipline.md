...
- **代价**：切片边界要自己找。shopifydesign M2/M3 累计 5 次边界错，全部被平衡检查当场抓出，单次定位约 5 分钟——`scripts/extract-source.mjs --balance-check` 已内置该检查（原理与实证见 §6.2 删桩流程），不必自写。
...
- [ ] 走字节切片的部分：sha256 守卫在位、切片器 `--check` 通过、别名表每行有解析依据；无任何"补 keyword"式切片（§2.2）
