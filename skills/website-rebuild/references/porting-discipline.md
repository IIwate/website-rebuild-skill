...
§4.1 那道反查门每轮都在生效，但它是**被动的**：一条登记只有在**被实际用到**的那一刻才会暴露它是错的。三轮连续实证说明这不够——**每一轮都抓到，而且抓到的是不同的错法**：
...
**最贵的是第三行**：M3b 做了一次更正，只更正了**内容**、没更正**坐标**，而**更正过的条目看起来是最可信的**——它刚被人认真读过。三条纪律因此成立：
...
3. **把复核变成阶段的固定动作，而不是事后补救。** 每个里程碑**开工时**（不是关账时）从三张表里**抽验 N 条**（实测 N = 3–5 就足以每轮见血），抽样优先级：① **最近被引用过**的（正在或即将被拿去下指令）；② **跨里程碑存活最久**、从未被任何门碰过的；③ **被用来下过硬指令**的——"门要断言 X 消失"这类，它错了会连带把门写错。复核结论**写进本轮日志**：核了哪几条、改了哪个字段、哪几条退回队列。
...
- [ ] **开工时抽验过 N 条既有登记**（含至少一条"上一轮刚更正过的"），逐字段核**内容 / 坐标 / 结论**三项，结果写进本轮日志（§4.2）
