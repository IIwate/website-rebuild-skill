...
| `scripts/probe-shim.js` | 确定性驱动 shim（接管整个熵面：rAF/timer/`performance.now`/`Date.now`/定种 `Math.random`/**IntersectionObserver**，手动泵到任意 t，双侧同位注入）。⭐ **IO 也是一个时钟**——浏览器按自己的节奏投递交叉记录，滚动揭示站因此无法冻结；接管后实测带宽 0.31 → **0.04**，门的可用阈值从 0.5 收到 0.1。⚠ **fork 补充**：接管把 IO 语义**归一化**了（忽略 `root`/`rootMargin`/`threshold`，一律按视口且 `ratio>0` 即触发）。两侧装同一个 shim，所以两侧本来就不同的 threshold 会被一起抹平成同一帧然后通过——**这是一条跨侧假绿**。观察器真的用到这些选项时控制台会打印被丢弃了什么；用 `?__probe&__noio` 可只退出 IO 接管（其余熵面仍冻结）做对照，若两次结论不同，差异是真的 | M(n-1) |
