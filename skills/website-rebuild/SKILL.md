...
| `scripts/cold-audit-modules.mjs` | **M(n) 冷头清点（模块化产物）**：逐模块对账，并查有没有**计算出来的 require**（认 webpack 与 Turbopack 两种工厂签名）。⛔⛔ **必须报出 `n/N examined` 并把覆盖率当判据**——它曾在一个模块都没查的情况下报绿；改成「仅 0 时报错」之后，又在查了 1/20 时报绿。⛔⛔ **fork 补第三次**：覆盖率门被写在 `else if` 里，于是只要命中任意一个可疑调用点，整条门被跳过——查了 1/5 且命中 1 个，输出里明明白白印着 `1/5 examined`，紧跟一行 `PASS`、退出码 0。**一道挂在 else 分支上的门不是门**；已改为无条件执行。⛔ **fork 另修**：打包器身份要**由容器（`MAP.container`）决定，不能从工厂是不是箭头函数去猜**——Webpack 5 在现代 target 下同样发射 `(module, exports, require) => {}`（Next.js 产物就是 `(e,t,r)=>{`），猜错时首参被当成 Turbopack 的 `ctx` 去搜 `e.i(`，一个 require 都搜不到却照样 `examined++`⛔ 实测抓到一处条件 require（`require(t ? "a" : "b")`）导致闭包少算两个模块，**而 9 个检查点逐像素全零毫无察觉**。⚠ 零依赖阶段分辨不了作用域遮蔽，所以只**判定**"有没有未移植模块被已移植模块 require"，疑似动态 require **列出来交人读** | M(n)（模块化打包产物） |
