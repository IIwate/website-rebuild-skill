 ...
 **依赖约束按阶段划: 源码化之前, 整条流水线零依赖.** 复刻项目从 Step 0 到 M(n)
不装任何东西;**到 M(n+1) 才获得 devDependencies**,因为作用域安全的分析需要真正的
parser(`@babel/parser` / `@babel/traverse`).这里放的就是那个阶段的工具.

 前面的阶段需要 parser 时,**外挂而不是 import**:spawn 一个固定版本的 npx
(见 `scripts/beautify-bundle.mjs`, `scripts/module-map.mjs`).

 **`scripts/` 里的任何检查都不许 import 这里的任何文件**--检查者不能是生产者
(`references/verification-gates.md` §2.1.2).两条约束都由 `scripts/verify-zerodep.mjs` 守.
...
| 脚本 | 用途 | 使用阶段 |
|---|---|---|
| `tools/assemble-static.mjs` | **像素检查两侧同经 serve.mjs**:把 `next build` 的 `.next/server/app/**.html` 摊成 `<route>/index.html`, `_next/static` 与 `public/*` 软链进静态树, 用 `serve --side rebuild` 提供服务--`next start` 侧不注入 probe-shim,镜像帧 BLANK/重建有画是冻结不对称不是差异(darkroom).只供对比, sweep 仍跑 next start | M(n-1)(C1 重构工程) |
