...
// checker report a spurious "own-build path" on every verbatim-only project.
...
// Include the file manifest and checker in the deliverable so copied files can
// be checked against their recorded digests without the repository. This checks
// file integrity, not runtime behavior or the completeness of the file list.
...
await writeFile(path.join(OUT, "package.json"), JSON.stringify({
  ...
  scripts: {
    ...
    // Include the server's configured external-host and stub-host mappings in the
    // deliverable. Omitting them can change resource routing or permit requests that
    // the in-repository setup handled locally (readable-source.md §2.4).
