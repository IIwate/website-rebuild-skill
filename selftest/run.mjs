...
// ------------------------------------------------- 3. lib/urlpath fixtures
{
  ...
  // A transform below an asset path remains a file even without a final extension.
  eq("urlpath — serveCandidates flat form first, exact (v0.1.72/v0.3.16)",
    serveCandidates("/f/1/x.jpg/m/110x110/filters:quality(70)", ""),
    ["/f/1/x.jpg@@m@@110x110@@filters:quality(70)", "/f/1/x.jpg/m/110x110/filters:quality(70)"]);
...
{
  const { createRefExtractor, createOffHostCensus, isTextRefSource, looksLikeAsset } = await import(path.join(SKILL, "scripts/lib/extract-refs.mjs"));
  ...
  const offHostExtract = createRefExtractor({ origin: "https://x.com", originHost: "x.com", assetHosts: new Set(["x.com"]), onOffHost: census.onOffHost });
  offHostExtract(`<img srcset="https://img.test/render?width=640 640w"><link href="https://meta.test/site.webmanifest"><a href="https://namespace.test/schema">schema</a>`);
  eq("extract — off-host census keeps long extensions and extensionless srcset evidence",
    census.summary().assetish.map(([host]) => host).sort(), ["img.test", "meta.test"]);
  truthy("extract — proxy assets and plain query pages share the crawler's classification",
    looksLikeAsset("https://img.test/render?url=%2Fbg.png") && !looksLikeAsset("https://x.com/about?tab=2"));
...
// ------------------------------------- 5. verify-mirror end-to-end fixture
{
  ...
  const run = () => {
    try { execFileSync(process.execPath, [path.join(SKILL, "scripts/verify-mirror.mjs"), "--mirror", M, "--gap-out", path.join(TMP, "closure-gap.txt")], { stdio: "pipe" }); return true; }
    catch { return false; }
  };
  truthy("verify-mirror — consistent mini mirror passes", run());
  for (const [file, type, box, brand, compatible] of [
    ["clip.mp4", "video/mp4", "ftyp", "isom", "iso2"],
    ["segment.m4s", "video/mp4", "styp", "msdh", "msix"],
    ["still.avif", "image/avif", "ftyp", "mif1", "avif"],
  ]) {
    const body = Buffer.alloc(24);
    body.writeUInt32BE(body.length);
    body.write(box, 4); body.write(brand, 8); body.write(compatible, 16);
    writeFileSync(path.join(M, file), body);
    files[`https://mini.test/${file}`] = { path: file, bytes: body.length, sha256: sha(body), type };
  }
  const writeLedger = () => {
    writeFileSync(path.join(M, "mirror-manifest.json"), JSON.stringify({ origin: "https://mini.test", files }));
    writeFileSync(path.join(M, "inventory.tsv"), "SHA256\tBYTES\tPATH\tURL\n" + Object.entries(files).map(([u, r]) => [r.sha256, r.bytes, r.path, u].join("\t")).join("\n") + "\n");
  };
  writeLedger();
  truthy("verify-mirror — MP4, fragmented MP4 and compatible-brand AVIF are accepted", run());
  files["https://mini.test/clip.mp4"].type = "image/jpeg";
  writeLedger();
  truthy("verify-mirror — an MP4 body cannot satisfy an image declaration", !run());
  files["https://mini.test/clip.mp4"].type = "video/mp4";
  files["https://mini.test/still.avif"].type = "video/mp4";
  writeLedger();
  truthy("verify-mirror — an AVIF body cannot satisfy a video declaration", !run());
  files["https://mini.test/still.avif"].type = "image/avif";
  writeLedger();
...
// --------------- 5c. module graph: turbopack merged/async shapes (v0.3.3)
{
  ...
  try {
    ...
    truthy("module-map — a cross-chunk async dependency remains visible outside the local closure",
      m111.externalRequires.includes("999999") && !m111.requires.includes("999999"));
    ...
    const incomplete = path.join(TMP, "alias-incomplete.json");
    writeFileSync(incomplete, JSON.stringify({ modules: ["555"] }));
    red("cold-audit — a required alias cannot hide an unported owning module",
      run("scripts/cold-audit-modules.mjs", ["--map", MMOUT, "--closure", incomplete]), /required by ported module/);
...
// ------------------------------ 5a2. webpack push container + token gates (v0.3.10)
{
  ...
  const WP3 = path.join(TMP, "wp-sparse.js");
  writeFileSync(WP3, `(self.webpackJsonp=self.webpackJsonp||[]).push([[1],[,,function(m,e,r){e.answer=r(5)},,,function(m){m.exports=42}]]);`);
  for (const [f, want, label] of [[WP1, ["87871"], "single-factory push chunk is 1 module, not the worker table (v0.3.11)"], [WP2, ["10", "20", "30"], "minified one-line chunk passes the char-overlap invariant (v0.3.11)"], [WP3, ["2", "5"], "sparse JSONP arrays retain ordinal module ids"]]) {
...
// Browser-dependent cases are covered in selftest/browser.mjs.
{
  ...
  // verify-offline — the static half of the zero-outbound gate, read from the SERVED bytes.
  {
    ...
    try {
      ...
      green("verify-offline — --url remains accepted by the shared CLI", run("scripts/verify-offline.mjs", ["--url", S.base + "/"]), /0 static outbound problem/);
